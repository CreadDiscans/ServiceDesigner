describe('Elements', function() {
    ...
    it('Copy Element', function() {
        createComponent('new')
        createElement('span')
        cy.get('#element-wrap').last().trigger('contextmenu')
        cy.get('#contextMenu').should($identifier => {
            expect($identifier.text().indexOf('Copy')).to.equal(-1)
            expect($identifier.text().indexOf('Paste')).to.equal(-1)
        })
        cy.get('.element-item').first().trigger('contextmenu')
        cy.get('#contextMenu').should($identifier => {
            expect($identifier.text().indexOf('Copy')).not.to.equal(-1)
        })
        cy.get('#contextMenu>div').eq(5).click()
        cy.get('#element-wrap').last().trigger('contextmenu')
        cy.get('#contextMenu').should($identifier => {
            expect($identifier.text().indexOf('Paste')).not.to.equal(-1)
        })
        cy.get('#contextMenu>div').eq(5).click()
        cy.get('.element-item').eq(1).should('have.text', 'span')
        cy.get('.element-item').eq(0).trigger('contextmenu')
        cy.get('#contextMenu>div').eq(6).click()
        cy.get('.element-item').first().trigger('contextmenu')
        cy.get('#contextMenu>div').eq(5).click()
        cy.get('.element-item').eq(1).trigger('contextmenu')
        cy.get('#contextMenu>div').eq(6).click()
    })
})