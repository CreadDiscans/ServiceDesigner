describe('Property', function() {
    beforeEach(function(){
        cy.visit('/')
    })

    function createComponent(name) {
        cy.get('div[id="components"]').trigger('mouseover')
        cy.get('span[id="icon-create-file"]>svg').click()
        cy.get('input[id="file-create-input"]').type(name+'{enter}')
        cy.get('.scrollarea-content').eq(0).click()
    }

    function createElement(name) {
        cy.get('#element-wrap').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('input[id="element-input"]').type(name+'{enter}')
    }

    it('Create String Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="Property"]').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('input[id="property-name-input"]').type('new')
        cy.get('input[id="property-value-input"]').type('value')
        cy.get('button[id="property-submit"]').click()
        cy.get('div[id="property-list"]').find('div').last().should('have.text', 'newstring')
        cy.get('input[id="property-name-input"]').should('have.value', '')
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('input[id="property-value-input"]').should('have.value', 'value')
    })

    it('Create Number Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="Property"]').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('input[id="property-name-input"]').type('new')
        cy.get('select[id="property-type-select"]').select('number')
        cy.get('input[id="property-value-input"]').type(100)
        cy.get('button[id="property-submit"]').click()
        cy.get('div[id="property-list"]').find('div').last().should('have.text', 'newnumber')
        cy.get('input[id="property-name-input"]').should('have.value', '')
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('input[id="property-value-input"]').should('have.value', '0100')
    })

    it('Create Boolean Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="Property"]').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('input[id="property-name-input"]').type('new')
        cy.get('select[id="property-type-select"]').select('boolean')
        cy.get('input[id="property-value-input"]').click()
        cy.get('button[id="property-submit"]').click()
        cy.get('div[id="property-list"]').find('div').last().should('have.text', 'newboolean')
        cy.get('input[id="property-name-input"]').should('have.value', '')
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('input[id="property-value-input"]').should('be.checked')
    })

    it('Create Object Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('div[id="property-value-object"]').find('textarea').focus().type('{backspace}{backspace}{{}background:white}', {force:true})
        cy.get('div[id="property-list"]').find('div').first().click()
        cy.get('div[id="property-list"]').find('div').last().should('have.text', 'styleobject')
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('div[id="property-value-object"]').find('.ace_scroller').should('have.text', '{background:white};')
    })

    it('Create Function Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="Property"]').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('input[id="property-name-input"]').type('new')
        cy.get('select[id="property-type-select"]').select('function')
        cy.get('button[id="property-submit"]').click()
        cy.get('div[id="property-list"]').find('div').last().should('have.text', 'newfunction')
    })

    it('Create Variable Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="Property"]').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('input[id="property-name-input"]').type('new')
        cy.get('select[id="property-type-select"]').select('variable')
        cy.get('input[id="property-value-input"]').type('this.state.value')
        cy.get('button[id="property-submit"]').click()
        cy.get('div[id="property-list"]').find('div').last().should('have.text', 'newvariable')
        cy.get('input[id="property-name-input"]').should('have.value', '')
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('input[id="property-value-input"]').should('have.value', 'this.state.value')
    })

    it('Delete Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="Property"]').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('input[id="property-name-input"]').type('new')
        cy.get('input[id="property-value-input"]').type('value')
        cy.get('button[id="property-submit"]').click()
        cy.get('div[id="property-list"]').find('div').last().trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').last().click()
        cy.get('div[id="property-list"]').find('div').should('have.length', 2)
    })

    it('Condition Object Property', function(){
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('div[id="property-value-object"]').find('textarea').focus().type('{backspace}{backspace}{{}background:red}', {force:true})
        cy.get('#property-detail-add-condition').click()
        cy.get('#property-detail-condition').type('true')
        cy.get('div[id="property-value-object"]').find('textarea').focus().type('{{}background:green}', {force:true})
        cy.get('#property-detail-add-condition').click()
        cy.get('#property-detail-condition').type('false')
        cy.get('div[id="property-value-object"]').find('textarea').focus().type('{{}background:blue}', {force:true})
        cy.get('div[id="property-list"]').find('div').first().click()
        cy.get('div[id="property-list"]').find('div').last().click()
        cy.get('div[id="property-value-object"]').find('.ace_scroller').should('have.text', '{background:red};')
        cy.get('.property-detail-badge').eq(1).click()
        cy.get('div[id="property-value-object"]').find('.ace_scroller').should('have.text', '{background:green};')
        cy.get('.property-detail-badge').eq(2).click()
        cy.get('div[id="property-value-object"]').find('.ace_scroller').should('have.text', '{background:blue};')
    })

    it('property name focus', function() {
        createComponent('new')
        createElement('div')
        cy.get('.scrollarea-content').eq(1).find('div').eq(0).click()
        cy.get('div[id="Property"]').trigger('contextmenu')
        cy.get('div[id="contextMenu"]>div').eq(0).click()
        cy.get('#property-name-input').should('have.focus')
    })
});